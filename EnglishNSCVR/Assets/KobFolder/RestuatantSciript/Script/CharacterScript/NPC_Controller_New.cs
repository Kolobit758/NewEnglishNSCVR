...
public class NPC_Controller_New : MonoBehaviour
{
    public bool isTest = false;
    public WordManager wordManager;
    public SitPosScript sitPosScript;
    public OrderNew orderNew;
    [Header("ref")]
    public ConverRulebase_Manager converRulebase_Manager;
    public ConverManager_New converManager;

    [Header("npc")]

    public List<SO_NPC> npcs = new List<SO_NPC>();
    public SO_NPC currentNpcData;
    public GameObject currrentNPC;
    public List<GameObject> npcSits = new List<GameObject>();
    ...
    public void SpawnNPC()
    {
        ...
        currentNpcData = npcs[randomNum];
        currrentNPC = Instantiate(currentNpcData.prefab, spawnPoint.position, Quaternion.identity);
    ...
    void WalkToPoint()
    {
        ...
        {
            Vector3 dir = (targetPoint.position - currrentNPC.transform.position).normalized;
            currrentNPC.transform.rotation = Quaternion.LookRotation(dir);
            isMoving = false;
    ...
    void NPCOutScene()
    {
        ...
        if (Vector3.Distance(currrentNPC.transform.position, outScenePoint.position) < 0.1f)
        {
            ...
            NPCgotoSit(currrentNPC);
            GameEvent.OnNPCLeaveFinished?.Invoke();
            moveState = 0;
            Debug.Log("move state : " + moveState);
        }
    }
    public void NPCgotoSit(GameObject npc)
    {
        npcSits.Add(npc);
        Transform sitPos = sitPosScript.GetSitPOS();
        npc.transform.position = sitPos.position;
    }
    ...
    {
        OrderData order = orderNew.RandomOrder();
