...
public class OrderNew : MonoBehaviour
{

    public MainDish currentOrder;
    public Gamesubmary gamesubmary;
    public NPC_Controller_New nPC_Controller_new;
    ...
    [SerializeField] SO_Topping defaultTopping;
    taste thisTaste;
    // ==================================================================================
    ...
    public void NewThickSpecial(bool isSpecial)
    {
        currentOrder.isSpecial = isSpecial;
        OnCostChanged();

    }
    public void NewChooseTatse(taste chooseTatse)
    {
        currentOrder.taste = chooseTatse;
    ...
    public void serve()
    {
        ...
        if (result)
        {
            nPC_Controller_new.PlayHappyFace();
            Debug.Log("Result: Success! NPC is happy.");
        }
        else
        {
            nPC_Controller_new.PlayAngryFace();
    ...
    bool isRightServe()
    {
        ...
        if (currentOrder.food.name != newOrder.food || currentOrder.food == null)
        {
            UpdateServiceScore(-2);
            Debug.Log("-2 service score");
            isRightServe = false;
        }
        if (currentOrder.beverage.name != newOrder.beverage || currentOrder.beverage == null)
        {
            UpdateServiceScore(-2);
            Debug.Log("-2 service score");
            isRightServe = false;
        }
        if (currentOrder.topping.name != newOrder.topping || currentOrder.topping == null)
        {
            UpdateServiceScore(-2);
            Debug.Log("-2 service score");
            isRightServe = false;
    ...
    public OrderData RandomOrder()
    {
        ...
        currentOrder.taste = taste.Normal;
        currentOrder.topping = defaultTopping;
